...
| Strict Pyright migration | Healthy through phase 4B | All twelve pinned Pyright configurations report 0 errors, and the full web lane passes 406 tests. Fresh candidate Docker/Playwright evidence confirms first-run claim, authenticated status `200` with `server.up`, and Dashboard, Settings, and Logs access before restoring the original app. | Fresh as of 2026-07-13. Phase 4A auth evidence still covers real-key zero-cookie remember `201`, Strict HttpOnly session cookies, dashboard reopen, and revoke `200`; the focused controller lane retains two exact-HEAD test-image `verify-file` timeout baselines and one intentional excessive-connections stress skip. Settings/admin/path-pairs/log/remaining serialization and global strict work remain pending. |
