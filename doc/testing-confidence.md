...
| Dockerized Angular build/test | Healthy | The supported Linux Chrome 151 lane passed 493/493, including the large-model regression that forbids eager full Immutable collection materialization during view rebuilds. Prior responsive file-list and reconciliation slices remain covered by this full collection. | Fresh as of 2026-08-03. Durable log: `tmp/pytest/large-model-initial-load-latency/verifier/angular-rerun.log`. |
...
| Downloaded-time and Smart Status sorting | Healthy | The established 519-test WSL and 81-spec Dockerized Angular coverage remains current. The 2026-08-08 start-time correction passed 15 focused WSL lifecycle/timestamp tests, and the Smart Status comparator passed 19/19 focused Dockerized Angular specs across active, actionable, terminal, null, and canonical-tie cases. Live Docker/Playwright proved Downloaded Newest moved a real active transfer from index 16 to 0; Smart Status kept a later active transfer at index 0 and the stopped row at index 1, then ordered completed rows by download recency across the remaining list with no browser errors. | Fresh as of 2026-08-08. Candidate image `sha256:17c08a2724256a3622d3b332f3ec728786d98e0498e11528145fd6d0359fa372`; browser evidence is in `tmp/pytest/download-start-time/playwright-live-start.json` and `playwright-smart-status-live.json`. |
