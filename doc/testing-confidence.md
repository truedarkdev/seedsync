...
| v0.8.6 historical upgrade and recovery | Healthy with own-install rollout gate | Fresh Docker/Playwright evidence proves a realistic migration with representative 0%, partial, and completed file states; a compact no-scroll recovery page; visually distinct running and stable completed checkpoints; an explicit Continue-to-bootstrap handoff and first-browser claim; and intentional empty path-pair persistence across restart. The retained `ship-20260801-r54` run still supplies the broader 21-row transfer, extraction, exact restore, pinned reboot-parity, and retained-audit evidence. | Fresh as of 2026-08-02. Independent correctness and security reviews passed, and the maintainer approved the complete repeated bootstrap → app → recovery → migration → bootstrap → app browser journey. The own-install v1.0 rollout gate remains open; no Docker-only deprecation/removal is implied. |
| Docker release publication | Unknown pending first hosted run | Local checks prove workflow syntax, immutable action and BuildKit references, safe secret transport, recorded staging-digest plumbing, exact version/`latest` manifest promotion, and fail-closed missing/mismatched digest handling; pinned `zizmor` v1.29.0 reports no medium-or-higher findings. A rebuilt Docker app served `/bootstrap` at HTTP 200, and Playwright rendered the SeedSync browser-access page without console errors or a lost-connection state. | Fresh as of 2026-08-02. GitHub-hosted CI and authenticated promotion to `truedarkdev/seedsync` have not run yet; do not treat the publication lane as healthy until the pushed commit passes CI and the first promoted digest/platform set is verified on Docker Hub. |
