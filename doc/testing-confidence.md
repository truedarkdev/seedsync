...
| v0.8.6 historical upgrade lab | Healthy with migration gap | The pinned 18-case fixture now has source-derived topology plus exact manifest/artifact/tamper validation. Stable Docker build/status/restart passed; Playwright dashboard, AutoQueue, Settings, Status, Sort, and post-restart checks passed with zero console/page errors. A fresh tracked transient run observed queued/downloading/extracting under the exact 45-second contract with no limitations; prior coordinator/startup evidence remains healthy. | Fresh as of 2026-07-21. Migration-only UI/API and direct/deb importer remain pending. |
