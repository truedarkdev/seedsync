...
| v0.8.6 historical upgrade lab | Healthy with migration gap | Migration coordinator core is implemented and healthy with final verifier coverage. Windows focused final: 30 passed, 2 privilege-dependent skips, and 10 subtests passed; WSL/Linux focused final: 27 passed, 5 skips, with POSIX symlink/root protections exercised; startup/config slice: 5 passed. Remote SSH `127.0.0.1:1234` and Docker app `127.0.0.1:8800` were healthy. Playwright reached the `/bootstrap` remembered-browser path with zero console/page errors; screenshot: `tmp/pytest/verifier-upgrade-final/bootstrap.png`. | Fresh as of 2026-07-21. Migration-only UI/API and direct/deb importer remain pending. |
