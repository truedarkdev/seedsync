...
| Queue authority and stop/resume | Healthy | Final focused WSL coverage passed 18 model-updater/controller-refresh tests. With the configured downloading scan interval left at 60,000 ms and the live Unraid connection topology reproduced at 4/4/16, the rebuilt Docker app forced active checkpoint scans about every two seconds, exposed a parser-valid LFTP sidecar, enabled Stop 2.08 seconds after Playwright first observed Downloading, and reached visible Stopped state after the browser click with no console or page errors. Earlier queue-authority coverage also proved coalesced Queue dispatch and exact path-pair-aware STOP routing. | Fresh as of 2026-08-08. Durable evidence is in `tmp/pytest/stop-checkpoint-verifier/`, including `active-stop-result.json`, `active-stop-enabled.png`, `active-stop-after.png`, and `representative-sidecar-after.log`; the one-connection diagnostic sidecar format remains outside this live 4/4/16 acceptance topology. |
...
| Docker release publication | Current image published and registry-checked | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` resolve to the same tested OCI index, `sha256:cd2c122a89dc12eb4ac8a620caccbf7083ba0887be0783f3196f007bfe5cb34a`, for `linux/amd64`, `linux/arm64`, and `linux/arm/v7`, with provenance attestations. Every runtime image carries version `0.9.0`, source `https://github.com/truedarkdev/seedsync`, and revision `29648b12548fdb2174eb8b69b9e5138ca4e692ca`. The default Dockerfile path now builds architecture-independent Angular assets on BuildKit's host platform, so multi-architecture releases no longer require the unavailable Node 24 ARMv7 image. The raw ARMv7 default path and the full three-platform build both passed without a prebuilt-Angular override. Before manifest-only promotion, the exact staging digest passed open-mode API and Playwright Dashboard, Settings, Logs, and Recovery rendering plus secure first-claim bootstrap checks; a fresh pull of published `latest` returned secure `/bootstrap` and unauthenticated `/server/status` 401 with no browser errors. | Fresh as of 2026-08-08. Both release tags were promoted atomically from the validated staging digest without rebuilding. Detailed evidence: `tmp/pytest/docker-publish-7b11b4399/`. The maintainer-owned Unraid movie retest remains the live rollout gate. |
