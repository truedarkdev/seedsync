...
| v0.8.6 historical upgrade and recovery | Healthy with maintainer UX gate | Fresh Docker/Playwright evidence proves a realistic migration with representative 0%, partial, and completed file states; a compact no-scroll recovery page; visually distinct running and stable completed checkpoints; explicit Continue; first-browser claim; and intentional empty path-pair persistence across restart. The retained `ship-20260801-r54` run still supplies the broader 21-row transfer, extraction, exact restore, pinned reboot-parity, and retained-audit evidence. | Fresh as of 2026-08-01. Independent correctness and security reviews passed. The maintainer's required second personal UX pass and own-install v1.0 rollout gate remain open; no Docker-only deprecation/removal is implied. |
