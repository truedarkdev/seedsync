...
| --- | --- | --- | --- | --- |
| Python web/auth backend | Healthy as of 2026-04-01 | The backend external-API auth foundation now has fresh local proof for scoped route gating, legacy-token restrictions, passwordless loopback UI-session behavior, the browser shell/static bootstrap boundary, `/server/stream` cookie alignment, and the controller/config web-handler paths that consume that auth model. | `src/python` pytest lanes for `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/unittests/test_web/test_web_app.py`, `tests/integration/test_web/test_web_app.py`, `tests/integration/test_web/test_handler/test_config.py`, and `tests/integration/test_web/test_handler/test_controller.py` | 2026-04-01 local Python runs; targeted reruns passed with junit artifacts under `src/python/tmp/verifier-auth-rerun-20260401-1/`, `tmp/pytest/verifier_trust_boundary/`, and `tmp/pytest/slice2-web-app.xml` |
...
| Angular API access UI/service lanes | Healthy as of 2026-04-01 | The new Settings-page API Access section has fresh targeted proof for the Angular service contract mapping, the API-access component interactions, and a host Settings-page regression pass. The supported local Docker UI bootstrap path now depends on the explicit trusted Docker-gateway runtime hook rather than a pure loopback transport assumption. | Targeted `src/angular` Karma lanes for `tests/unittests/services/settings/api-access.service.spec.ts`, `tests/unittests/pages/settings/api-access.component.spec.ts`, and `tests/unittests/pages/settings/settings-page.component.spec.ts` | 2026-04-01 local targeted runs; service lane passed `5` tests, component lane passed `5` tests, and the existing settings-page regression passed `1` test; artifacts `tmp/pytest/api-access-service.spec.out.log`, `tmp/pytest/api-access-component.spec.out.log`, and `tmp/pytest/settings-page.component.spec.out.log` |
