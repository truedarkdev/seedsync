...
| Strict Pyright migration | Healthy through phase 3C | Pinned strict controller-scan, controller-process, model, common, and default basic Pyright report 0 errors. The current scan/controller-refresh slice passed 88 tests covering remote JSON/path-pair handling, multi-path scanning, queue publication, and correlated breadcrumbs; prior process lifecycle, model, and common malformed JSON/IPC, legacy-marker, and equality-protocol evidence remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; extract remains next, followed by orchestrator, web, and global strict phases. |
