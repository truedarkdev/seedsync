...
| Legacy Protractor/e2e | Flaky as of 2026-04-01 | The Dockerized lane now gets through compose startup, configure, remote/Selenium readiness, and browser session bootstrap. A bounded WebDriver 4 bridge and route-bootstrap helper contain the earlier Angular testability-sync failure on the targeted dashboard/settings navigation path, and the dashboard selector/data hardening in this task now leaves the targeted dashboard spec slice with only the intentionally skipped download-progress branch unresolved in the Docker compose fixture. The broader legacy lane still needs a fresh full-lane rerun before it can be treated as healthy. | `src/docker/test/e2e/compose.yml` plus targeted legacy Protractor specs under `src/e2e/tests` | 2026-04-01 targeted local Docker reruns: settings spec assertion passed without the old `both angularJS testability and angular testability are undefined` failure, but teardown still exited `199`; dashboard targeted rerun originally failed later with `NoSuchElementError`, `StaleElementReferenceError`, and row-count/index assumptions, then the hardened dashboard spec ran as 7 passed / 1 pending in the compose fixture because there is no downloading row to exercise; artifacts `tmp/pytest/e2e-rerun-settings-spec.log`, `tmp/pytest/e2e-rerun-dashboard-spec.log`, and `tmp/pytest/e2e-rerun-route-bootstrap-restore-probe.log` |
...
- Stop-resume browser/e2e lane: the backend stop-state slice now has fresh
  ...
  testability-sync failure on the targeted dashboard/settings path; the
  targeted dashboard selector/data brittleness is now hardened in the
  dashboard spec slice, but the broader legacy e2e lane still needs a fresh
  full-lane rerun before it can be treated as healthy.
