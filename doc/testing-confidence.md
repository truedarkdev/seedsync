...
| v0.8.6 historical upgrade lab | Healthy with migration gap | Stable lane: `make upgrade-v086-preflight`, `make upgrade-v086-build RUN_ID=<id>`, `make upgrade-v086-start RUN_ID=<id> HOST_PORT=<port>`, `make upgrade-v086-status RUN_ID=<id> HOST_PORT=<port>`, and `make upgrade-v086-restart RUN_ID=<id> HOST_PORT=<port>`. Dedicated transient lane: `make upgrade-v086-build-transient RUN_ID=<id>`, `make upgrade-v086-start-transient RUN_ID=<id> HOST_PORT=<port>`, then `make upgrade-v086-transient RUN_ID=<id> HOST_PORT=<port>`. The tracked manifest drives pinned historical backend/frontend states, frozen lab npm resolution, AutoQueue/persist markers, full nested/special-name trees, and scanner exclusion; transient controls are isolated to transient-mode runs. | Fresh as of 2026-07-20: revision-normal2 full-tree status/restart and revision-transient bounded lftp/extractor evidence observed QUEUED, DOWNLOADING, DOWNLOADED, and EXTRACTING with no synthesized states. Migration engine/UI work is not implemented. |
