...
| Strict Pyright migration | Healthy through phase 3D | Pinned strict controller-extract, controller-scan, controller-process, model, common, and default basic Pyright report 0 errors. Extract passed 67 unit tests plus 28 real-7z Docker integration tests with no skips; prior scan/controller-refresh, process lifecycle, model, and common malformed JSON/IPC, legacy-marker, and equality-protocol evidence remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; controller-root/orchestrator remains next, followed by web and global strict phases. |
