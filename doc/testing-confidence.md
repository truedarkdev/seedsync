...
| v0.8.6 historical upgrade lab | Healthy with migration gap | Canonical lane: `make upgrade-v086-preflight`, then `make upgrade-v086-build RUN_ID=<id>`, `make upgrade-v086-start RUN_ID=<id> HOST_PORT=<port>`, and `make upgrade-v086-status RUN_ID=<id> HOST_PORT=<port>`. Pinned historical backend/frontend build with a frozen lab-owned npm resolution; legacy and SSH fixtures stay on a per-run internal network behind a loopback-only proxy; first remote scan is healthy; Playwright reaches the real historical UI. | Fresh as of 2026-07-20. Migration engine/UI work is not implemented; this lane proves the retained legacy fixture and upgrade foundation only. |
