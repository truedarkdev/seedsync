...
| Strict Pyright migration | Healthy through phase 3F | All ten configured Pyright lanes report 0 errors. Current core and auto-queue evidence covers 313 tests, with an additional persisted-shape test covering six malformed collection subcases. The service-enabled controller integration lane is 50 passed, 2 exact-HEAD baseline failures from the test image's `verify-file` stop/resume timeouts, and 1 intentional excessive-connections stress skip; no candidate-specific failure remains. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Strict/core evidence is fresh as of 2026-07-13; Docker/authenticated browser and original-app restoration evidence remains current from 2026-07-12. Web and global strict phases remain pending. |
