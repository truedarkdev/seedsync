...
| Docker release publication | Current image published and registry-checked | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` resolve to the same tested OCI index, `sha256:fece3e96269ccd0ca19f01dbd14a58e9daaa77e5afc670e53afb0d08c5d507ea`, for `linux/amd64`, `linux/arm64`, and `linux/arm/v7`, with provenance attestations. Every runtime image carries version `0.9.0`, source `https://github.com/truedarkdev/seedsync`, and revision `8e1a4f501af48d8cdc9f63f2cb5b036d7004964a`. The exact staging digest passed secure startup, first claim, the approved sidebar version footer, and clean fresh-root reconciliation: with pair AutoQueue off, byte-identical `existing.bin` was adopted while `remote-only.bin` waited; after AutoQueue was enabled and the container restarted, only `remote-only.bin` queued and downloaded; both files retained their hashes and mtimes through a second restart with no further Queue command. A fresh pull of published `latest` returned `/bootstrap` 200, unauthenticated `/server/status` 401, first claim 201, authenticated status 200, `v0.9.0`, and no browser errors. | Fresh as of 2026-08-09. Both release tags were promoted from the validated staging digest without rebuilding. Detailed evidence: `tmp/pytest/docker-publish-8e1a4f501/`. The maintainer-owned Unraid path-pair consolidation and live validation remain the rollout gate. |
...
| Path-pair deployment continuity | Healthy | Final WSL verifier coverage passed 403 AutoQueue, Controller, ModelUpdater, scanner, and path-pair handler tests. Rebuilt image `sha256:9068c3958c829041884754b7c3f47b74768db38352d184e06ba4b7367f9dfe0f` reconciled an isolated retained library through Docker and Playwright with only `/mounts/movies` mounted: an added remote-only file queued, transferred, and auto-deleted, while exact, zero-byte, local-only, partial, and conflicting fixtures stayed unchanged. The retained pair ID, path, and AutoQueue setting survived relocation and restart, with `/downloads/movies` absent. | Fresh as of 2026-08-09. Durable evidence: `tmp/pytest/path-pair-deployment-continuity-final-verifier/final2/`. |
