...
| Python unit/backend | Healthy | Current focused backend evidence covers controller/model-update, auto-queue, row `727` web security-header/auth behavior, row `873`/`875` move-completion ordering and pair-scoped local refresh, and the row `906` `remote_python_path` config/remote-scanner and redaction hardening slice. | Fresh row `727` verifier/final evidence is from 2026-06-24: WSL web/auth slice passed, diff hygiene passed, Docker rebuild/start and HTTP reachability passed, live headers were checked on `/bootstrap` and `/does-not-exist`, and Playwright smoke/screenshot evidence was captured at `tmp/verifier-browser-smoke.png`. Row `873`/`875` verifier/final evidence is also from 2026-06-24: focused `test_scanner_process.py` and `test_multi_path_scanner.py` coverage passed, targeted controller slices passed, `git diff --check` stayed clean aside from LF/CRLF warnings, and Docker-served Playwright screenshot evidence was captured at `tmp\\verifier\\seedsync-home-rerun.png`. The local app container later exited because of an unrelated remote `/tmp/scanfs` helper issue (`SyntaxError: Non-UTF-8 code starting with '\\xd8' in file /tmp/scanfs`), so long-lived container health was not reverified. Row `906` worker self-checks passed on 2026-06-24 for backend config parsing, remote-scanner command construction, controller passthrough, remote-detail redaction, and narrow Angular settings specs. Broader WSL full-suite evidence remains available from 2026-04-08. |
