...
| Strict Pyright migration | Healthy through phase 4A | All eleven configured Pyright lanes report 0 errors. Current web-auth evidence covers 102 auth-store, web-app, web-app-job, and web integration tests, including malformed persisted auth shapes and trusted first-run redirect-without-session behavior. Atomic Docker/Playwright validation confirmed first-run claim, a real API key in a second zero-cookie browser returning remember `201` with a Strict HttpOnly cookie, dashboard reopen, revoke `200`, and restoration of the original app. | Fresh as of 2026-07-13. The focused controller integration lane remains 50 passed with two exact-HEAD test-image `verify-file` timeout baselines and one intentional excessive-connections stress skip; web lifecycle/handlers and global strict phases remain pending. |
