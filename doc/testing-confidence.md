...
| Strict Pyright migration | Healthy through phase 3E2 | Pinned strict auto-queue, notifier, controller-extract, controller-scan, controller-process, model, common, and default basic Pyright report 0 errors. Auto-queue passed 79 tests covering queue authority, state eligibility, path-pair identity and opt-out, command origin, extract/delete ordering, and persistence; prior notifier, extract, scan/controller-refresh, process lifecycle, model, and common evidence remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; model updater remains next, followed by model builder, controller, web, and global strict phases. |
