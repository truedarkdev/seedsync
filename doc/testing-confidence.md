...
| Strict Pyright migration | Healthy through phase 4C | All thirteen pinned Pyright configurations report 0 errors, and the full web lane passes 408 tests. Fresh deterministic Docker/Playwright recovery used a unique recovery version and confirmed first-run claim, API-key create/revoke, path-pairs and authenticated status `200`, Dashboard, Settings, and Logs access, then restoration of the original app. | Fresh as of 2026-07-13. Prior auth evidence still covers real-key zero-cookie remember `201`, Strict HttpOnly session cookies, and dashboard reopen; the focused controller lane retains two exact-HEAD test-image `verify-file` timeout baselines and one intentional excessive-connections stress skip. Final log/stream/notification web surfaces and global strict work remain pending. |
