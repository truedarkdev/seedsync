...
| --- | --- | --- | --- | --- |
| Python web/auth backend | Healthy as of 2026-04-02 | The backend external-API auth foundation now has fresh local proof for scoped route gating, legacy-token restrictions, passwordless loopback UI-session behavior, cookie-auth write hardening, the browser shell/static bootstrap boundary, `/server/stream` cookie alignment, the controller/config web-handler paths that consume that auth model, the Slice 6 follow-up hardening for body-based config writes, same-origin bootstrap tightening, and trusted forwarded-origin handling, plus the final security-audit hardening that removed legacy-token path-pair reads, suppressed pre-bootstrap admin-cookie minting, and split browser bootstrap trust between direct loopback and trusted-proxy loopback-origin paths. | `src/python` pytest lanes for `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/unittests/test_web/test_web_app.py`, `tests/integration/test_web/test_web_app.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/integration/test_web/test_handler/test_controller.py`, the slice-4 rerun for `tests/unittests/test_web/test_web_app.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/integration/test_web/test_handler/test_auto_queue.py`, the Slice 6 rerun for `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_web_app.py` after restoring the protected body-write checks plus the bootstrap revision rerun for `tests/unittests/test_web/test_handler/test_admin_handler.py` and `tests/unittests/test_web/test_web_app.py`, and the final security slice rerun for `tests/integration/test_web/test_handler/test_path_pairs.py`, `tests/unittests/test_web/test_web_app.py`, and `tests/unittests/test_web/test_handler/test_admin_handler.py` | 2026-04-02 local Python runs; targeted reruns passed with junit artifacts `tmp/pytest/slice4-python.xml`, `tmp/pytest/slice6-python.xml`, `tmp/pytest/slice6-bootstrap-revision.xml`, and `tmp/pytest/bootstrap_pathpair_security.xml`; rebuilt local Docker runtime also rejected direct non-loopback bootstrap hosts with `HTTP/1.0 403 Forbidden` and no `Set-Cookie` |
