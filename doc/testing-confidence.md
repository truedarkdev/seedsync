...
| v0.8.6 historical upgrade and recovery | Healthy with own-install rollout gate | Fresh candidate image `sha256:fff4820fea25187517a983f8a315a86c5989bbbc9d1fa996c59489d6db61f4f1` passed named-host migration access with no origin-allowlist configuration: Playwright/Edge observed `seedsync.example.test` 303 → `/migration` 200, visible migration UI, status `required`, and no browser errors; a valid-CSRF mismatched-origin mutation returned 403 and left the checkpoint idle. Full changed-file WSL verification passed 54 + 25 tests. The published amd64 candidate's private Unraid `fuse.shfs` UID/GID `99:100` preflight and retained `ship-20260801-r54` transfer, extraction, restore, reboot-parity, and audit evidence remain current. | Fresh as of 2026-08-02. Independent correctness, security, and verifier gates passed. The maintainer explicitly accepts reachable arbitrary named authorities and DNS rebinding as part of the brief unauthenticated v0.8.6 checkpoint; exact same-origin, CSRF, confirmation, backup-before-mutation, loopback package defaults, and the untrusted-network warning remain. Retesting the newly published image on the maintainer-owned live movie instance remains the rollout gate. |
| Docker release publication | Unknown pending hosted/multi-architecture promotion | The amd64 `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` tags both resolve to digest `sha256:4d6f770e41e067e43d97978a5a5f366351dcc7332462ea2fcccc92553ef29f3c` with revision `11f76062e76a33ac8dfb015a06d8da10d900b1fd`. The labeled release image has the same root filesystem layers as the fresh Docker/Playwright-verified named-host candidate; Docker Hub inspection and a digest-qualified pull confirmed matching tags plus the expected source, revision, and `0.9.0` version labels. | Fresh as of 2026-08-02. Per maintainer policy, `latest` follows the current published version. The maintainer-owned Unraid pull/retest, hosted tested-digest lane, and remaining architectures are still required before treating publication as healthy. |
...
| Docker runtime/process supervision | Healthy | The rebuilt candidate passed the full config-root contract in strict and legacy non-root modes, including UID/GID derivation, idempotence, unsafe-tree rejection, and both attacker race barriers. A live `--user 1101:1102` container ran `tini` as PID 1, kept `/config` and its per-user `TMPDIR` at `0700`, created the browser proof at `0600`, and served the first-run bootstrap page to Playwright with HTTP 200 and no console/page errors or lost-connection state. The exact published image then passed against a private Unraid `fuse.shfs` copied config as `99:100` without changing its aggregate content hash. | Fresh as of 2026-08-02. The maintainer-owned live movie swap remains part of the own-install rollout gate. |
