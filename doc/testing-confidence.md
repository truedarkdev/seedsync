...
| Strict Pyright migration | Healthy through phase 3B | Pinned strict controller-process, strict-model, strict-common, and default basic Pyright report 0 errors; 23 path-safe delete and validation lifecycle tests passed. Prior model and common malformed JSON/IPC, legacy-marker, and equality-protocol evidence remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; scan/extract, orchestrator, web, and global strict phases remain pending. |
