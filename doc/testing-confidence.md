...
| Python unit/backend | Healthy | Current focused backend evidence covers controller/model-update, scanner/delete/extract race handling, SSH child cleanup, WSGI request timeouts, auto-queue, move-completion ordering, and remote-scanner/config redaction. | Fresh as of 2026-07-10: the rapidcopy hardening verifier/final slice passed 145 focused tests with 56 environment-gated skips, plus fresh Docker API, Playwright dashboard, and SSE checks. Broader WSL full-suite evidence remains available from 2026-04-08. |
| Outbound notifications | Healthy | 21 lifecycle tests plus 94 broader tests cover provider-aware auth/redaction, SSRF-safe pinned transport, bounded queue/retry behavior, and download/extraction/successful-remote-delete events for Generic Webhook and Apprise API. | Fresh as of 2026-07-10: independent UI/security checks passed, exact `lscr.io/linuxserver/apprise-api` downstream-receiver delivery was confirmed, and Docker/Playwright passed on final image `sha256:5ad5b83cdf93ee07b23168341e7288e8782f7498124cd6f0819d2879f0d702e1`. |
...
| Dockerized Angular build/test | Healthy | Dockerized Angular verifier path passes after the current frontend compatibility updates, including 414/414 Angular tests and a production build. | Supported frontend closure lane. Fresh as of 2026-07-10. |
