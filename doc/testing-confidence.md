...
| v0.8.6 historical upgrade lab | Healthy with migration/publication gaps | Windows backup/restore final suite: 26 passed, 27 skipped, 47 subtests; coordinator: 34 passed, 2 skipped, 22 subtests; WSL backup/restore: 53 passed. Fresh pinned legacy `restorefinal2` Docker cycle completed backup -> mutation -> supported offline CLI restore -> migration-required re-detection with exact hidden/nested/arbitrary file hash preservation. Standalone Playwright against the Docker-served app passed all UI/API acceptance criteria with no console/page errors. | Fresh as of 2026-07-25. Product Start remains intentionally disabled because migration execution is not enabled; direct/deb importer and migration-only UI/API remain pending. Separate `virtualized-files-list` publication blocker remains tracked. |
