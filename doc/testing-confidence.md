...
| v0.8.6 historical upgrade and recovery | Healthy with own-install rollout gate | The retained `ship-20260801-r54` transfer, extraction, restore, reboot-parity, and audit evidence remain current. The current open-first-run candidate also passed the maintainer's live failure shape with legacy `local_path = downloads`, Docker `--user 99:100`, and no browser-source setting: Edge/Playwright through an ordinary nginx proxy reached the claim page with HTTP 200, claimed with 201, opened `/dashboard`, and used authenticated APIs with 200; a fresh browser received no session and `/server/status` returned 401. Persisted legacy `trusted_browser_bootstrap_remote_addrs` input is accepted, discarded, and absent after the next save. | Fresh as of 2026-08-02. Independent correctness, verifier, and security closure passed. First-run migration and administrator claim are intentionally open to any reachable client until the first successful claim; unfinished setup must remain on a trusted network, proxies must overwrite forwarded host/proto headers, and HTTPS-proxied sessions receive `Secure` cookies. Retesting the newly published image on the maintainer-owned live movie instance remains the rollout gate. |
| Docker release publication | Healthy with own-install rollout gate | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` both resolve to the tested manifest `sha256:5ff8a5226b7c68fb32c6d6f978d2c7bd7e3c8a8dc0f39aa2fb2ba18162f5fb70`. Docker Hub inspection confirmed `linux/amd64`, `linux/arm64`, and `linux/arm/v7`; a digest-qualified amd64 pull and legacy `--user 99:100` entrypoint smoke passed, and labels identify source `truedarkdev/seedsync`, revision `e66e564148fddedd00ce801377b38bf22aad4db9`, and version `0.9.0`. | Fresh as of 2026-08-02. Per maintainer policy, `latest` follows the current published version. The maintainer-owned Unraid pull/retest remains the rollout gate. |
