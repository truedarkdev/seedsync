...
| Dockerized Angular build/test | Healthy as of 2026-04-01 | The Dockerized Angular lane still passes cleanly after the service-layer RxJS modernization slice, so the Angular 21 / Node 24 Docker verifier path remains the supported frontend closure lane. | `src/docker/test/angular/compose.yml` default verifier path plus Angular build images | 2026-04-01 local Docker proof; exact `docker compose -f src/docker/test/angular/compose.yml up --build --abort-on-container-exit --exit-code-from tests` exited `0` with `TOTAL: 296 SUCCESS`; service-layer RxJS modernization candidate verified on that lane |
