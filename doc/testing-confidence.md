...
| --- | --- | --- | --- | --- |
| Python web/auth backend | Healthy as of 2026-04-01 | The backend external-API auth foundation now has fresh local proof for scoped route gating, legacy-token restrictions, passwordless loopback UI-session behavior, and the controller/config web-handler paths that consume that auth model. | `src/python` pytest lanes for `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/unittests/test_web/test_web_app.py`, `tests/integration/test_web/test_web_app.py`, `tests/integration/test_web/test_handler/test_config.py`, and `tests/integration/test_web/test_handler/test_controller.py` | 2026-04-01 local Python runs; targeted reruns passed with junit artifacts under `src/python/tmp/verifier-auth-rerun-20260401-1/` and `tmp/pytest/verifier_trust_boundary/` |
