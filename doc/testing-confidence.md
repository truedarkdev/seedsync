...
| --- | --- | --- | --- | --- |
| Python web/auth backend | Healthy as of 2026-04-04 | The backend external-API auth foundation now has fresh local proof for scoped route gating, invalid/obsolete token rejection behavior, passwordless loopback UI-session behavior, cookie-auth write hardening, the browser shell/static bootstrap boundary, `/server/stream` cookie alignment, the controller/config web-handler paths that consume that auth model, the Slice 6 follow-up hardening for body-based config writes, same-origin bootstrap tightening, trusted forwarded-origin handling, the final bootstrap-exchange hardening, and the new LAN-first remembered-browser API-key flow: trusted-local first-run claim UI, remembered browser sessions bound to API key identity and current key state, recovery-version reopening, atomic concurrent first-visitor claim handling, and restart-durable remembered-browser cookie persistence through normal browser close/reopen. | `src/python` pytest lanes for `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/unittests/test_web/test_web_app.py`, `tests/integration/test_web/test_web_app.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/integration/test_web/test_handler/test_controller.py`, the slice-4 rerun for `tests/unittests/test_web/test_web_app.py`, `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/integration/test_web/test_handler/test_auto_queue.py`, the Slice 6 rerun for `tests/unittests/test_web/test_handler/test_config_handler.py`, `tests/integration/test_web/test_handler/test_config.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_web_app.py`, the final security slice rerun for `tests/integration/test_web/test_handler/test_path_pairs.py`, `tests/unittests/test_web/test_web_app.py`, and `tests/unittests/test_web/test_handler/test_admin_handler.py`, the final bootstrap-exchange hardening rerun for `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, `tests/unittests/test_web/test_web_app.py`, and `tests/integration/test_web/test_web_app.py`, plus the remembered-browser follow-up reruns for `tests/unittests/test_common/test_config.py`, `tests/unittests/test_web/test_auth_store.py`, `tests/unittests/test_web/test_handler/test_admin_handler.py`, and `tests/unittests/test_web/test_web_app.py` | 2026-04-02 local Python runs with junit artifacts `tmp/pytest/slice4-python.xml`, `tmp/pytest/slice6-python.xml`, `tmp/pytest/slice6-bootstrap-revision.xml`, `tmp/pytest/bootstrap_pathpair_security.xml`, and `tmp/pytest/verifier-bootstrap-exchange/junit.xml`; 2026-04-03 targeted local Python reruns passed for the remembered-browser lane (`27 passed`, `69 passed`, `25 passed`, plus targeted reruns for atomic claim and compatibility slices), and live Docker checks confirmed baseline `/bootstrap`, recovery-version reopening, `first-api-key`, and `/server/browser/v1/remember` behavior; 2026-04-04 targeted auth/admin pytest and rebuilt live Docker + Playwright validation confirmed `seedsync_ui_session` now issues `HttpOnly; Max-Age=43200; SameSite=Strict` and persists across full browser close/reopen back to `/dashboard` |
...
| Angular sidebar path-pair refresh UI | Healthy as of 2026-04-04 | The sidebar refresh-rendering lane now has fresh proof for the hard-refresh hydration bug where path-pair buttons could stay hidden until another interaction. The component now re-enters Angular before applying path-pair state, and the live Docker app shows `Series` and `Movies` immediately on direct load and reload. | Targeted `src/angular` Karma lane for `tests/unittests/pages/main/sidebar.component.spec.ts`, plus live browser proof against `http://127.0.0.1:8800/dashboard`, `http://127.0.0.1:8800/dashboard/Series`, and `http://127.0.0.1:8800/dashboard/Movies` | 2026-04-04 targeted Angular rerun passed with the new outside-zone refresh regression and existing multi-/single-path-pair visibility cases; rebuilt live Docker + Playwright validation with an authenticated UI session confirmed sidebar path-pair buttons were present immediately at direct load and after reload with no extra interaction |
| Angular API access UI/service lanes | Healthy as of 2026-04-04 | The Settings-page API Access section now has fresh proof for the shared confirm-modal click path, the API-access component interactions, and the live Docker-served browser flow for create, edit, rotate, revoke, legacy disable, legacy clear, the revoked-key lifecycle slice, and the new bulk revoked-key cleanup flow: revoked keys hidden by default, reveal toggle, locked revoked rows, permanent delete, repeat-revoke rejection, and the new `Delete all` action with confirmation, active-key preservation, and partial-failure refresh coverage. The supported local Docker UI bootstrap path still depends on the explicit trusted Docker-gateway runtime hook rather than a pure loopback transport assumption. | Targeted `src/angular` Karma lane for `tests/unittests/services/utils/modal.service.spec.ts`, targeted `src/angular` Karma lanes for `tests/unittests/services/settings/api-access.service.spec.ts`, `tests/unittests/pages/settings/api-access.component.spec.ts`, and prior `tests/unittests/pages/settings/settings-page.component.spec.ts`, plus live browser proof against `http://127.0.0.1:8800/settings` | 2026-04-02 local proof; targeted backend pytest, Angular/Karma proof, and live Docker/browser validation all contributed to the revoked-key lifecycle evidence; shared modal regression ran in the ChromeHeadless Angular suite with `TOTAL: 308 SUCCESS` from `tmp/angular-modal-service-spec.log`; rebuilt live Docker browser validation confirmed real pointer clicks on API Access rotate/revoke modals and exercised create, edit, revoke, legacy disable, legacy clear, and revoked-key lifecycle behavior with screenshots under `tmp/slice5-*`; earlier 2026-04-01 targeted API-access service/component/settings-page artifacts remain valid at `tmp/pytest/api-access-service.spec.out.log`, `tmp/pytest/api-access-component.spec.out.log`, and `tmp/pytest/settings-page.component.spec.out.log`; 2026-04-04 targeted Angular reruns passed for `tests/unittests/pages/settings/api-access.component.spec.ts` and `tests/unittests/services/settings/api-access.service.spec.ts`, and rebuilt live Docker + Playwright validation confirmed `Delete all` stays hidden until revoked keys are shown, opens scoped confirmation copy, removes revoked keys, preserves active keys, and leaves single-key delete behavior intact |
