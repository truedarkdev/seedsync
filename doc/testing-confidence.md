...
| Strict Pyright migration | Healthy through phase 3E1 | Pinned strict notifier, controller-extract, controller-scan, controller-process, model, common, and default basic Pyright report 0 errors. Notifier passed 27 tests covering safe `NotificationError` boundaries, network policy, retries, queue drops, shutdown, and redacted failures; prior extract, scan/controller-refresh, process lifecycle, model, and common evidence remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; auto-queue remains next, followed by model updater/builder, controller, web, and global strict phases. |
