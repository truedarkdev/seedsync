...
| Final-move failure/retry | Healthy | Terminal move-failure surfacing and retry, performed-move-only success metadata, no-op Downloaded preservation, lifecycle reset/pruning, symlink-safe path resolution, monotonic pending-completion progress, and exact-ID stale active-snapshot quarantine are covered. Genuine destination truncation and remote replacement remain visible. | Fresh as of 2026-08-08: WSL verifier lanes passed 13 completion/final-move controller tests, 17 model-updater tests, and 151 model-builder tests. Rebuilt Docker/Playwright row-select → Queue proof captured 71 DOM samples and 70 SSE events with monotonic progress through 99% → 100% → Move succeeded, no browser errors, a complete destination with no staging copy, and terminal 100% state after restart. |
