...
| v0.8.6 historical upgrade and recovery | Healthy with own-install rollout gate | Fresh candidate image `sha256:aba75dfa82589fe49ea87ae67f5e991441e1b583a6e9f06bec968a9ae1350506` passed the live failure shape: legacy `local_path = downloads`, Docker `--user 99:100`, migration Continue, retained-backup preservation, current-path repair to `/downloads`, NSS-backed OpenSSH/remote shell and scanfs setup, and a running normal controller. Playwright/Edge reached the first-run claim page through an explicitly trusted test proxy with HTTP 200 and no console/page errors; the unchanged blank proxy-trust case returned the expected bootstrap 403. The retained `ship-20260801-r54` transfer, extraction, restore, reboot-parity, and audit evidence remain current. | Fresh as of 2026-08-02. Independent correctness, security, and verifier gates passed. The migration checkpoint's intentionally broad hostname admission remains separate from the explicit `[General] trusted_browser_bootstrap_remote_addrs` administrator-claim boundary. Retesting the corrected published image on the maintainer-owned live movie instance remains the rollout gate. |
...
| Docker runtime/process supervision | Healthy | The config-root contract remains current in strict and legacy non-root modes. Fresh candidate `sha256:aba75dfa82589fe49ea87ae67f5e991441e1b583a6e9f06bec968a9ae1350506` additionally passed unknown numeric identities `99:100` and `1101:1102`: owner-private nss_wrapper files supplied `pwd`/group records, OpenSSH configuration succeeded, the entrypoint remained idempotent, and a normal migrated controller stayed running. `/config` and the per-user runtime home remained owner-private; arbitrary configured path pairs and the retained migration backup were not rewritten. | Fresh as of 2026-08-02. The maintainer-owned live movie swap remains the own-install rollout gate; proxy-sourced first-browser claim still requires an explicit trusted proxy source IP/CIDR. |
