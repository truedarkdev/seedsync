...
| v0.8.6 historical upgrade and recovery | Healthy with own-install rollout gate | Fresh Docker/Playwright evidence proves a realistic migration with representative 0%, partial, and completed file states; a compact no-scroll recovery page; visually distinct running and stable completed checkpoints; an explicit Continue-to-bootstrap handoff and first-browser claim; and intentional empty path-pair persistence across restart. The exact published amd64 candidate also passed a non-mutating UID/GID `99:100` preflight against the private copied movie config through Unraid `fuse.shfs`; its aggregate file hash was identical before/after and `/config` ended mode `0700`. The retained `ship-20260801-r54` run still supplies the broader 21-row transfer, extraction, exact restore, pinned reboot-parity, and retained-audit evidence. | Fresh as of 2026-08-02. Independent correctness and security reviews passed for the migration/recovery implementation, and the maintainer approved the complete repeated bootstrap → app → recovery → migration → bootstrap → app browser journey. The maintainer-owned live movie swap remains the own-install v1.0 rollout gate; no Docker-only deprecation/removal is implied. |
| Docker release publication | Unknown pending hosted/multi-architecture promotion | The amd64 `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` tags both resolve to digest `sha256:36325c36344a665dd138786cb182ee7004453d91c8eedb67f936a57b3a520a8a` with revision `92d7d054d547c3714af2ebdd21bfc6e2fd627eea`. Docker Hub inspection and a fresh Unraid pull agreed on the digest, architecture, source, revision, and version labels; local Docker/Playwright and private Unraid `fuse.shfs` preflight evidence are green. | Fresh as of 2026-08-02. Per maintainer policy, `latest` follows the current published version. The hosted tested-digest and remaining architecture lanes are still required before treating publication as healthy. |
