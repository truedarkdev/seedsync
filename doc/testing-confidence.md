...
| Docker release publication | Current image published and registry-checked | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` resolve to the same tested OCI index, `sha256:38f7272065c56702992cfd6358e394b6e084de668f7246c2d61dad57a55d3fca`, for `linux/amd64`, `linux/arm64`, and `linux/arm/v7`, with provenance attestations. Every runtime image carries version `0.9.0`, source `https://github.com/truedarkdev/seedsync`, and revision `3f8051351ea5847a0ef70e0891eff928eb80f659`. Before manifest-only promotion, the exact staging digest passed open-mode API lifecycle/cross-origin checks and Playwright Dashboard, Settings, Logs, and Recovery rendering; a fresh pull of published `latest` returned secure `/bootstrap` and unauthenticated `/server/status` 401 with no browser errors. | Fresh as of 2026-08-08. Both release tags were promoted atomically from the validated staging digest without rebuilding. Detailed evidence: `tmp/pytest/docker-publish-3f8051351/`. The maintainer-owned Unraid movie retest remains the live rollout gate. |
