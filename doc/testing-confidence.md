...
| Docker release publication | Current image published and registry-checked | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` resolve to the same tested OCI index, `sha256:f0d22c5af8e113c1852110cb65dd2ffdaecdca52f961e164fbae0d2a8038a838`, for `linux/amd64`, `linux/arm64`, and `linux/arm/v7`, with provenance attestations. Every runtime image carries version `0.9.0`, source `https://github.com/truedarkdev/seedsync`, and revision `7ece7168dce8724b113df686463860324f217710`. Before manifest-only promotion, the immutable staging digest passed secure first-run checks and a real Docker/Playwright Stop → Resume → Stop flow that held a 58% progress floor through 13 observations to 63% while preserving live speed/ETA and producing no browser errors. A fresh pull of published `latest` returned secure `/bootstrap`, unauthenticated `/server/status` 401, and the expected first-run copy without browser errors. | Fresh as of 2026-08-08. Both release tags were promoted from the validated staging digest without rebuilding. Detailed evidence: `tmp/pytest/docker-publish-7ece7168d/`. The maintainer-owned Unraid movie retest remains the live rollout gate. |
