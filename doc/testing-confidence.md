...
| v0.8.6 historical upgrade lab | Healthy with rollout gate | Retained run `ship-20260801-r54` passed all 21 Docker/Playwright rows: realistic pinned v0.8.6 baseline; migration with retained private backup and migrated settings/data; browser claim and remembered-browser restart; real scan, transfer/resume, extraction and safe operations; Angular 72/72; exact offline config/download restore; and pinned v0.8.6 browser/API/model parity after reboot. The retained audit passed across 213 files with no findings. | Fresh as of 2026-08-01. Independent verifier and reviewer passed. The maintainer's own-install v1.0 rollout gate remains separate; no Docker-only deprecation/removal is implied. |
