...
| Python unit/backend | Healthy | Targeted controller unit coverage for path-pair refresh/runtime state rebuild behavior plus breadcrumb trace collector retention/filter, disabled-backlog, reset, and cross-process gate coverage, controller failure-marker, and auto-queue summary coverage. | Fresh as of 2026-04-07. |
| Python integration/controller | Healthy | Targeted controller and web-handler integration coverage for the breadcrumb retrieval path, including since-version, filtered retrieval, authenticated reset, and config-gate sync, plus the initial model path. | Fresh as of 2026-04-07. |
