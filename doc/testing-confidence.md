...
| v0.8.6 historical upgrade and recovery | Healthy with maintainer UX gate | Docker/Playwright proves a realistic v0.8.6 migration, blank-version first claim, authenticated direct recovery, offline restore back to `/migration`, retry, second claim, restart health, remote scan, and unchanged download/mount inventories. The retained `ship-20260801-r54` run still supplies the broader 21-row transfer, extraction, exact restore, pinned reboot-parity, and retained-audit evidence. | Fresh as of 2026-08-01. Independent correctness and security reviews passed. The maintainer's personal recovery-flow approval and own-install v1.0 rollout gate remain open; no Docker-only deprecation/removal is implied. |
