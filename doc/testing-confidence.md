...
| --- | --- | --- | --- |
| Python web/auth backend | Healthy | Final focused verifier passed 162 tests with 22 subtests. A rebuilt Docker candidate preserved the default first-run `/bootstrap` flow and added an explicit legacy mode that reached `/dashboard` without cookies, allowed ordinary status/SSE/config actions, rejected invalid credentials and unauthenticated admin/recovery access, preserved first-claim plus second-browser API-key remember/admin access, rejected invalid env values, and restored auth when the same config volume restarted with env `0`. The prior 1701-test WSL sweep remains the broad backend baseline. | Fresh as of 2026-08-03. Durable evidence: `tmp/pytest/browser_auth_disable_unit_final.xml`, `tmp/pytest/auth_verify_final.json`, `tmp/pytest/auth_enabled_lifecycle.json`, and `tmp/pytest/auth_claim_remember.json`. |
