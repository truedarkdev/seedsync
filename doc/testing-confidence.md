...
| v0.8.6 historical upgrade and recovery | Corrective candidate verified; republish and own-install retest pending | The retained `ship-20260801-r54` transfer, extraction, restore, reboot-parity, and audit evidence remain current. The maintainer's live Unraid migration then exposed a bounded handoff defect: if the first normal-startup `/bootstrap` probe remains pending behind a long-lived reverse proxy, the published `0.9.0` UI stays indefinitely on `Starting SeedSync...`. A Docker-served Playwright comparison reproduced that exact state with the published bundle; the corrective candidate cancelled the held request after about one second, exposed non-secret numbered probe requests plus bounded console diagnostics, retried successfully, and reached `/bootstrap` in about 1.26 seconds. Focused Angular tests passed 25/25, and the exact final Docker image served the first-claim page with HTTP 200 and no browser errors while unauthenticated `/server/status` remained 401. | Fresh as of 2026-08-02. First-run migration and administrator claim remain intentionally open to any reachable client until the first successful claim; unfinished setup must stay on a trusted network. The corrective image is not published yet, and the maintainer-owned movie retest remains the rollout gate. |
| Docker release publication | Published image has a corrective candidate pending | `truedarkdev/seedsync:0.9.0` and `truedarkdev/seedsync:latest` still resolve to manifest `sha256:0e754c774ddcd4bd078902b1e224e90f5193ad62e76d72f3be8560980cf9d652` for `linux/amd64`, `linux/arm64`, and `linux/arm/v7`, with revision `5d11ee2600b8fd0c6d4333842e585517bd51f49a`. Its UID 99 compatibility, proxy claim, session, and postclaim-denial evidence remains valid, but the own-install rollout confirmed the migration-to-bootstrap pending-request hang described above. The local corrective candidate has focused Angular, production-build, Docker-served comparison, and full-image browser evidence; it has not replaced the published manifest. | Fresh as of 2026-08-02. Do not describe the current Docker Hub tags as containing the handoff fix until a new exact multi-platform manifest is published and inspected. Per maintainer policy, `latest` must move with that corrected publication. |
...
| Docker runtime/process supervision | Healthy | The config-root contract remains current in strict and legacy non-root modes. Published image `sha256:0e754c774ddcd4bd078902b1e224e90f5193ad62e76d72f3be8560980cf9d652` passed unknown numeric identity `99:100`: owner-private nss_wrapper files supplied the runtime passwd/group record, `ssh -G` succeeded, and the exact image remained healthy through the no-allowlist proxy claim flow. Earlier `1101:1102`, idempotence, normal-controller, path-pair, and retained-backup evidence remains current because this candidate does not alter the entrypoint/config-root implementation. | Fresh as of 2026-08-02. The maintainer-owned live movie swap remains the own-install rollout gate; first-run browser claiming no longer requires proxy-source configuration. |
