...
| Strict Pyright migration | Healthy through phase 3A | Pinned strict-model, strict-common, and default basic Pyright report 0 errors; 47 model tests and 142 model-builder consumer tests passed, including the unrelated-object equality protocol probe. Phase 2 malformed JSON/IPC and legacy-marker recovery coverage remains green. Docker rebuild plus authenticated Playwright Dashboard, Settings, and Logs checks passed, then the original app was restored. | Fresh as of 2026-07-12; controller subsystem/orchestrator, web, and global strict phases remain pending. |
