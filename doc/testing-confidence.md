...
| Final-move failure/retry | Healthy | Terminal move-failure surfacing and retry, performed-move-only success metadata, no-op Downloaded preservation, lifecycle reset/pruning, and contained symlink-safe path resolution have focused backend/frontend coverage plus live Docker/API and Playwright browser proof. | Fresh as of 2026-07-11; independent reviewer and security gates passed. Known duplicate-root extracted-marker and native-Windows separator expectations are unrelated and remain outside this lane. |
