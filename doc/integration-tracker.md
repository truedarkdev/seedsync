...
- Notes:
  ...
  - Chunk `128-167` is dispositioned: 4 `already integrated`, 20 `covered elsewhere`, 10 `intentionally skipped`, 3 `needs integration` rows (`129`, `142`, `143`), 3 adapted rows (`137`, `141`, `165`), 0 `needs area reopen`, 0 `needs new integration task`, and 0 `maintainer decision needed`; row `142` remains queued because only its remote-path/tilde-delete and shell-detection/scanner-install subfeatures are adapted so far.
  - Next nitrobass24 work: implement the remaining chunk `128-167` queue before opening row `168`; row `129` needs a separate scanfs architecture design because a literal amd64-only pin conflicts with multi-arch image artifacts while a target-platform default conflicts with the common x86_64 seedbox copy path, row `142` still needs optional-password/config-sentinel follow-up after its remote-path and shell-detection subfeatures landed, and row `143` covers optional auto-delete-after-download behavior.
