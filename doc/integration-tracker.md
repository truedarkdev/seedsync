...
- Last dispositioned fork-unique row: `329` (`5cb2a7a9793908186f73236a870ecb35d8e43671`)
...
- Resume/refresh: current frozen manifest is dispositioned through row `329`. Row `328` multi-select/bulk operations is covered by the existing local `FileSelectionService` + `bulk-actions-bar` + bulk endpoint architecture; focused Angular specs and live Docker/Playwright validation confirmed row checkboxes, select-all visible, and the bulk action bar with selected count. Row `329` scanfs JSON serialization is already integrated through local commits `0c88994b` and `5ad98ce1`; verifier confirmed focused Python tests, live Docker `scan_fs.py` JSON output, Docker `SystemFile` round-trip, Docker `RemoteScanner` JSON decode, and no `pickle` usage in the scan transport path. Next fork-unique manifest row to process is `330`. A later refresh is needed because `nitrobass24/develop` advanced to `ab7fb1aea644712d50c2fabf19ae48744185c2d4` after the original frozen tip `38d6ef22d36b6a75c164bc754bac9cd2842e8722`.
- Notes:
