...
| `PB-002` | closed | docker-image build is now self-contained, with Angular assets and `scanfs` built inside the final image build instead of pulled from staged export images. Multi-arch publish validation still belongs to `PB-009`, not this closure. | Main-session verification passed: `docker build -f src/docker/build/docker-image/Dockerfile --target seedsync_run -t seedsync-pb002 .`, `make -n docker-image`, `make -n docker-image-release`, and `docker run --rm seedsync-pb002 ... /app/scanfs --help` confirmed the staged export dependency was removed and the built image contains runnable bundled assets. | `2026-03-12` |
| `PB-003` | closed | Angular build/test compatibility now uses a modern Node 20 baseline, Node-20-safe Sass/Karma tooling, and a current Chromium test image path without changing Angular 4.x app/framework dependencies. | Main-session verification passed: `docker build -t seedsync-pb003-angular -f src/docker/build/deb/Dockerfile --target seedsync_build_angular .` and `make run-tests-angular` after capturing the resolved Node 20 toolchain in `src/angular/package-lock.json`. | `2026-03-12` |
| `PB-004` | closed | the broad Angular/RxJS replatform lane is intentionally deferred for this conservative integration fork now that PB-003 restored a stable Angular 4 + Node 20 compatibility baseline. | Local proof: sampled dirty Angular files reduce to whitespace/EOL churn under `git diff -w`, while no coherent semantic modernization batch or dependency/workspace migration is staged locally. | `2026-03-12` |
| `PB-005` | closed | GLIBC verification now covers both bundled PyInstaller artifact paths that this fork ships: the built `.deb` payload and the exported `scanfs` binary. | Main-session verification passed: `src/docker/test/verify_glibc.sh build/seedsync_0.8.6_amd64.deb 2.31`, direct-binary verification on extracted packaged `scanfs`, `make -n verify-deb-glibc`, and `make -n verify-scanfs-glibc`; `.github/workflows/master.yml` now runs `make scanfs`, `make verify-deb-glibc`, and `make verify-scanfs-glibc` in the build-deb job. | `2026-03-12` |
| `PB-006` | closed | the active `ubu2004` stage/deb lane now carries cgroups-v2-friendly setup, writable cgroup access, line-ending-safe runtime scripts, and a valid systemd launch command for the stage container path. | Local proof: `src/docker/stage/deb/Dockerfile` now normalizes copied scripts and launches systemd through `/bin/bash -c`, `src/docker/stage/deb/entrypoint.sh` now safely `exec \"$@\"`, `docker compose -f src/docker/stage/deb/compose.yml config` passes, and the deb E2E workflow still configures Docker with `default-cgroupns-mode: host`; live Docker runtime execution remained unavailable in this session because `/var/run/docker.sock` access is denied. | `2026-03-12` |
| `PB-007` | closed | Active deb E2E policy surfaces now align on `SEEDSYNC_OS=ubu2004` as the enforced Ubuntu 20.04 lane. | Local proof: `.github/workflows/master.yml` deb E2E matrix now uses only `ubu2004`; `Makefile` builds only the Ubuntu 20.04 stage image for `tests-e2e-deps` and rejects non-`ubu2004` deb E2E requests; `doc/DeveloperReadme.md` now documents `SEEDSYNC_OS=ubu2004` as the active Ubuntu 20.04 deb E2E lane. | `2026-03-12` |
| `PB-008` | closed | the legacy E2E stability slice now has longer startup/configure wait budgets, tighter suite timeouts, and low-risk page-object settle waits for dashboard and AutoQueue actions. | Local proof: `src/docker/test/e2e/run_tests.sh`, `src/docker/test/e2e/configure/setup_seedsync.sh`, `src/e2e/conf.ts`, `src/e2e/tests/dashboard.page.ts`, and `src/e2e/tests/autoqueue.page.ts` now apply conservative timeout/wait hardening without changing the suite structure. | `2026-03-12` |
| `PB-009` | closed | a shared arch-to-platform mapping now drives Docker-image E2E pulls and the remote container platform selection, and the workflow matrix gives the ARM64 lane its own native runner entry. | Local proof: `src/docker/test/resolve_platform.sh`, `Makefile`, and `src/docker/test/e2e/compose.yml` now resolve `amd64`, `arm64`, and `arm/v7` through one mapping before the E2E pull/Compose path runs, while `.github/workflows/master.yml` gives `arm64` its own `ubuntu-24.04-arm` matrix lane. | `2026-03-12` |
| `PB-010` | closed | the tag-triggered GitHub release path now uses maintained `gh release` commands instead of deprecated release actions while preserving `.deb` asset publication. | Local proof: `.github/workflows/master.yml` now creates or reuses the tag release with `gh release` and uploads the built `.deb` asset directly from the downloaded artifact. | `2026-03-12` |
| `PB-011` | closed | closed by `ac8b6447`, `f4a62a75`, `754b6a50`. | Explicit local proof: `ac8b6447`, `f4a62a75`, `754b6a50`. | `2026-03-12` |
| `PB-012` | closed | no active implementation gap is currently evidenced, including after the PB-001 Bottle `0.12.25` refresh. | Main-session 3.11 `WebApp` import smoke remained clean after the Bottle refresh, so the old pin-specific rationale is now superseded. | `2026-03-12` |
| `PB-013` | closed | common-module coverage has landed, and the upstream skip decorators were intentionally not adopted because the targeted tests pass in the supported Docker Python harness. | Local proof: targeted common-module coverage now exists in `test_constants.py`, `test_error.py`, `test_localization.py`, `test_context.py`, and `test_types.py`; Docker verification also passed for `test_extract_archive_overwrites_existing` and the web stream handler suite without adding upstream skip markers. | `2026-03-12` |
| `PB-014` | closed | the broader bounded-state lane now includes lightweight controller-level memory monitoring on top of the already-landed stream-queue and downloaded-state protections, while the rolled-back persisted LRU/set redesign remains intentionally not adopted on this branch. | Main-session verification passed: `python3 -m py_compile` on the touched controller files, `cd src/python && poetry run python -m unittest tests.unittests.test_controller.test_memory_monitor tests.unittests.test_controller.test_controller.TestController.test_log_memory_usage_reports_current_controller_collection_sizes`, plus earlier local proof for bounded `StreamQueue` coverage and downloaded-state pruning/cache invalidation. | `2026-03-12` |
...
| `PB-016` | closed | files/options lifecycle subscriptions now tear down cleanly on destroy, so the remaining files-page subscription cleanup gap is closed. | Local proof: `52402b26` updated `src/angular/src/app/pages/files/file-options.component.ts` and `src/angular/src/app/tests/unittests/pages/files/file-options.component.spec.ts` with explicit unsubscribe coverage after destroy. | `2026-03-12` |
| `PB-017` | closed | bulk endpoints now validate payload size, reject overlapping bulk requests, keep bounded callback waits, and preserve timeout/failure summaries across retries. | Local proof: `85cbe899` landed bounded callback waits, and `52402b26` completed the remaining bulk limiter and oversized-payload coverage in `src/python/web/handler/controller.py` and `src/python/tests/integration/test_web/test_handler/test_controller.py`. | `2026-03-12` |
| `PB-018` | closed | SSE transport now includes heartbeat keepalive registration plus one-event-per-handler fair yielding with short per-event pacing, so busy producers no longer monopolize the multiplexed stream. | Local proof: heartbeat registration and fair-yield/pacing behavior now live in `src/python/web/web_app_builder.py`, `src/python/web/handler/stream_heartbeat.py`, and `src/python/web/web_app.py`, with focused coverage in `src/python/tests/unittests/test_web/test_web_app.py`; Angular Docker verification also passed in `make run-tests-angular` (`251 SUCCESS`). | `2026-03-12` |
| `PB-019` | closed | client-side SSE dispatch now guards orphaned events, keeps reconnect attempts from stacking timers/subscriptions, and tolerates malformed status/model payloads without crashing stream consumers. | Local proof: hardened registry/model/status handling now lives in `src/angular/src/app/services/base/stream-service.registry.ts`, `src/angular/src/app/services/files/model-file.service.ts`, and `src/angular/src/app/services/server/server-status.service.ts`, with focused coverage in the matching Angular specs; Docker verification passed in `make run-tests-angular` (`251 SUCCESS`). | `2026-03-12` |
| `PB-020` | closed | single-action web handler callbacks now use a bounded 30 second wait and return `504` on timeout while preserving existing success/error mapping. | Local proof: `f288e3ab` landed the timeout guard in `src/python/web/handler/controller.py`, with targeted controller handler timeout coverage in `src/python/tests/integration/test_web/test_handler/test_controller.py`. | `2026-03-12` |
...
Pending:
- Audit reopen: `91fa0101` and `5e39fdeb` exposed a common-module unit-test slice (`test_constants.py`, `test_error.py`, `test_localization.py`, `test_context.py`, `test_types.py`) that has now landed locally; the related skip-policy follow-up was later resolved by Docker verification instead of importing upstream skip decorators.
...
- Dirty-worktree triage on `2026-03-12` showed the apparent 168-file modified tree was mostly CRLF/EOL churn. `git diff --ignore-cr-at-eol --shortstat` reduced the substantive unstaged set to six files: `src/angular/package.json`, `src/angular/karma.conf.js`, `src/angular/src/app/tests/unittests/services/files/view-file-filter.service.spec.ts`, `src/angular/src/app/tests/unittests/services/files/view-file-sort.service.spec.ts`, `src/docker/build/deb/Dockerfile`, and `src/docker/test/angular/Dockerfile`. That PB-003 Angular/Karma/Node20 compatibility slice was recovered in `18cdfffa`, and the wider dirty tree should still be treated as EOL churn unless intentionally doing line-ending cleanup.
...
- `0cb3228` was originally skipped because the then-current Bottle `0.12.19` path did not reproduce the `_stop_flag` attribute conflict; after PB-001 refreshed Bottle to `0.12.25`, a clean Python 3.11 `WebApp` import smoke still did not surface an active gap, so PB-012 is closed with no reopened work.
...
- adapted the warning-cleanup intent from `58af9ee8` as a small follow-up batch so `test_job_status_parser.py` no longer emits invalid-escape `SyntaxWarning`s during `py_compile`, without changing parser behavior or broadening the transfer/LFTP scope
