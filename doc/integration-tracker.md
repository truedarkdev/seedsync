...
| `PB-005` | partially needed | the repo now has an explicit `.deb` GLIBC verification path, but the broader cross-path compatibility policy still remains separate. | Local proof: `src/docker/test/verify_glibc.sh`, `make verify-deb-glibc`, and the `build-deb` workflow job now extract the built `.deb`, inspect bundled `seedsync`/`scanfs` binaries with `readelf`, and fail if the active Ubuntu 20.04+ verification ceiling (`GLIBC 2.31`) is exceeded. | `2026-03-12` |
| `PB-006` | partially needed | the active `ubu2004` stage/deb lane now carries cgroups-v2-friendly setup, writable cgroup access, and active-lane systemd container masking, but this environment did not exercise the Docker runtime path. | Local proof: `src/docker/stage/deb/compose.yml` now runs the active lane as a privileged service with writable `/sys/fs/cgroup`, the `ubuntu-20.04-systemd` image masks `systemd-resolved`, the local setup helper skips legacy host mounting on cgroups v2, and the deb E2E workflow now configures Docker with `default-cgroupns-mode: host` before the stage/deb run. | `2026-03-12` |
...
| `PB-014` | partially needed | bounded stream queues, safe downloaded-file pruning, and downloaded-set cache invalidation have landed, but persisted downloaded-file caps were rolled back as unsafe with the current set-only persistence and the broader memory-monitor lane still remains. | Local proof: `StreamQueue` is bounded with focused coverage in `src/python/tests/unittests/test_web/test_stream_queue.py`; controller/model-builder hardening now covers stale downloaded-id pruning plus downloaded-set cache invalidation in `src/python/tests/unittests/test_controller/test_controller.py` and `src/python/tests/unittests/test_controller/test_model_builder.py`, while the attempted cap trimming was removed after review. | `2026-03-12` |
