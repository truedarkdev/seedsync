...
- Queue / status: chunk `168-207` is clear. Row `183` Files-page status sorting is adapted locally as a first-class `Smart Status` sort option. Row `184` is intentionally skipped because the Settings page has a custom, well-defined layout. Rows `172` and `187` are folded into the existing future theming/visualization track and should not be re-asked as nitrobass24 chunk decisions.
- Resume when new upstream appears: run `git fetch --no-tags nitrobass24 --prune`, verify the branch tip, then open row `208` in the next exact 40-row contiguous chunk, oldest-to-newest; rows `1-87` are baseline accounting rows and rows `88-207` are already dispositioned.
- Notes:
  ...
  - Next nitrobass24 work: open row `208` in the next exact 40-row contiguous audit oldest-to-newest.
