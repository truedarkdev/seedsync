...
- Last dispositioned fork-unique row: `328` (`e7144fae4b4b125b1a3c7da7075236dc0badecb4`)
...
- Resume/refresh: current frozen manifest is dispositioned through row `328`. Row `328` multi-select/bulk operations is covered by the existing local `FileSelectionService` + `bulk-actions-bar` + bulk endpoint architecture; focused Angular specs and live Docker/Playwright validation confirmed row checkboxes, select-all visible, and the bulk action bar with selected count. Next fork-unique manifest row to process is `329`. A later refresh is needed because `nitrobass24/develop` advanced to `ab7fb1aea644712d50c2fabf19ae48744185c2d4` after the original frozen tip `38d6ef22d36b6a75c164bc754bac9cd2842e8722`.
- Notes:
