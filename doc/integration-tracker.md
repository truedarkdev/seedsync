...
- Last dispositioned fork-unique row: `345` (`27cf0e259a5cdb7998ca76c55ea1d2c32b09623e`)
...
- Queue / status: chunk `288-327` normal implementation queue and security-hardening area reopen lane are empty. Row `331` webhook notifications needs area reopen for a holistic notification system rather than a standalone import of the early minimal webhook patch; future work must handle visible settings UI approval plus webhook URL privacy/redaction. Row `332` historical log query/search needs a new integration task because it adds a persisted log API and visible log search/filter UI, must preserve `/server/*` auth scope enforcement, and needs adaptation to the legacy Angular/log-stream stack. Row `333` CI branch publish support belongs in the existing CI topology area reopen with rows `273`, `282`, and `286`, because upstream targeted a now-absent `feat/v0.13.0` branch and this repo currently uses master/tag-oriented publishing. Rows `302`, `306`, and `307` security hardening are adapted locally in `3e2485e5`, with a non-blocking residual that external extractors can still perform absolute-path writes outside staged extraction without a stronger sandbox. Row `327` structured JSON logging is adapted locally in `f3f5c790`. Row `336` Paste server removal is adapted locally in `8b21f9f0`, using a bounded stdlib WSGI adapter with raw-path preservation, stream/normal request capacity split, fail-safe stream classification, and request logging. Row `337` multi-path-pair support is covered by the local evolved path-pair architecture, including path-pair migration, pair-aware file identity, controller/runtime routing, API wiring, and Angular file identity state. Rows `338`, `339`, and `340` path-pair review fixes need area reopen together to reconcile malformed path-pair config semantics, explicit path-pair handler JSON object and field validation, persisted `enabled`/`auto_queue` boolean strictness, remove-pair-not-found signaling under the local `PathPairError` contract, exact migration sentinel checks, manager concurrency/locking intent, and local bounded web-server teardown semantics against the evolved local architecture. Row `341` path-pairs merge is covered by rows `337`-`340`; it adds no independent implementation delta beyond that merge payload and branch reconciliation. Rows `342` and `344` Python scanfs primary-scanner/PyInstaller removal and self-contained remote scan script work need area reopen: local JSON scan transport is partly covered, but scanner execution, `scan_fs.py` self-containment, `--scanfs` semantics, PyInstaller dependency/build wiring, Docker/deb packaging, compose defaults, docs, and files-page delete-confirm UX need a reconciled scanner/runtime packaging and UI-alignment slice with Linux/Docker verifier coverage. Row `343` stale Paste reference cleanup is covered by row `336`/`8b21f9f0`; upstream modal localization removal is intentionally not taken because local Angular still uses the `Localization.Modal.DELETE_*` strings in active modal delete confirmation flows. Row `345` direct extractor subprocess replacement needs a focused extraction area reopen because local extraction remains patool-backed; adapt the direct ZIP/TAR/RAR/7Z handlers and dependency removal while preserving the local staging/failure behavior and addressing the known absolute-path external-extractor residual from rows `302`, `306`, and `307`. Rows `309` and `310` Docker/package dependency trimming are adapted locally in `034832ed`. Row `304` eager settings-service initialization and directly required config/autoqueue/logging hardening are adapted locally in `518422ee`. Row `303` Angular CSP-friendly build output is adapted locally in `c1a3a77c`. Row `300` restricted remote scanfs install fallback behavior is adapted locally in `f1a407c5`. Rows `295`, `297`, and `301` explicit-UMASK/runtime-permissions behavior are adapted locally in `46cd02fe`. Row `267` delete-action and remote-delete cleanup behavior is adapted locally in `cedadd93`; row `268` Docker UMASK environment support is adapted locally in `9d9c795a`. Rows `256` and `258` extraction dispatch failure reporting and staging fallback archive lookup are adapted locally in `ae841fb1df6c88a8d8fd60641abcca5681588b6f`; pair-specific staging fallback coverage remains a separate follow-up if needed. Rows `253` and `260` unrar/RAR5 Docker packaging support are adapted locally in `aa46ccd3814de04198e41ecdabf1d1d1d6354161` without upstream's deb12-specific pin or changelog edit. Row `249` parser and controller lifecycle/zombie-handling behavior is adapted locally across `f7d52a4e537013fd52e8dfaf70f5885a954cddaa` and `22908425df8bd2cf586f39990b56bdc581dd2e2a`. Row `250` incomplete staged-directory guarding is adapted locally in `1df4fa8601a08dd2a0a960cde19b2ad4dd3efae1`. Row `279` SSH password log redaction is adapted locally in `a9d02db2feaf03923207fc394356eef126c3c6e0`; a separate non-blocking security follow-up remains for `general.webhook_secret` config API serialization. Rows `273`, `282`, and `286` relate to CI/build workflow topology; rows `275` and `287` relate to docs/UMASK/Unraid docs path; row `285` relates to Docker/runtime packaging optimization; rows `261`, `277`, `292`, and `321` are website-only dependency rows and are intentionally skipped because this workspace lacks the website subtree.
- Resume/refresh: current frozen manifest is dispositioned through row `345`. Row `328` multi-select/bulk operations is covered by the existing local `FileSelectionService` + `bulk-actions-bar` + bulk endpoint architecture; focused Angular specs and live Docker/Playwright validation confirmed row checkboxes, select-all visible, and the bulk action bar with selected count. Row `329` scanfs JSON serialization is already integrated through local commits `0c88994b` and `5ad98ce1`; verifier confirmed focused Python tests, live Docker `scan_fs.py` JSON output, Docker `SystemFile` round-trip, Docker `RemoteScanner` JSON decode, and no `pickle` usage in the scan transport path. Row `330` is covered by the same local selection architecture because local `ViewFile` uses `isSelected`/`FileSelectionService`, not upstream's `isChecked` helper shape. Row `331` webhook notifications is queued as an area reopen because local SeedSync has no notification subsystem yet and the upstream patch touches backend event delivery, config serialization, and visible settings UI. Row `332` historical log search is queued as a new integration task because live stream filtering only partly overlaps with persisted log-file history. Row `333` CI branch publish support is queued with the CI topology area reopen rather than adapted standalone. Row `334` is covered elsewhere because it is merge topology only, reconciling row `333` with earlier row `314` ancestry and introducing no standalone patch behavior. Row `335` is covered elsewhere because it is a CI retrigger commit with the same tree as row `334` and no file-level delta. Row `336` Paste server removal is adapted locally in `8b21f9f0`. Row `337` multiple path pairs support is covered by the local evolved path-pair architecture. Rows `338`, `339`, and `340` path-pair review fixes are queued with the path-pair hardening area reopen. Row `341` path-pairs merge is covered by rows `337`-`340`. Rows `342` and `344` Python scanfs primary-scanner/PyInstaller removal and self-contained remote scan script work are queued as one scanner/runtime packaging area reopen. Row `343` stale Paste reference cleanup is covered by row `336`, and modal localization strings are intentionally retained because they are still live locally. Row `345` direct extractor subprocess replacement is queued as an extraction area reopen. Next fork-unique manifest row to process is `346`. A later refresh is needed because `nitrobass24/develop` advanced to `ab7fb1aea644712d50c2fabf19ae48744185c2d4` after the original frozen tip `38d6ef22d36b6a75c164bc754bac9cd2842e8722`.
- Notes:
