...
| `PB-004` | conditional future follow-up only | the broad Angular/RxJS replatform lane is intentionally deferred for this conservative integration fork now that PB-003 restored a stable Angular 4 + Node 20 compatibility baseline. | Local proof: sampled dirty Angular files reduce to whitespace/EOL churn under `git diff -w`, while no coherent semantic modernization batch or dependency/workspace migration is staged locally. | `2026-03-12` |
...
| `PB-012` | conditional future follow-up only | no active implementation gap is currently evidenced, including after the PB-001 Bottle `0.12.25` refresh; revisit only if a real `_stop_flag` regression appears. | Main-session 3.11 `WebApp` import smoke remained clean after the Bottle refresh, so the old pin-specific rationale is now superseded. | `2026-03-12` |
