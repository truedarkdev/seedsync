...
The first-browser administrator claim is a separate trust boundary from the migration checkpoint. A reverse proxy must be explicitly trusted through `[General] trusted_browser_bootstrap_remote_addrs`, using the narrowest proxy source IP or CIDR that reaches SeedSync (for example, `172.19.0.26/32`). Do not enter the end user's LAN address when the application actually sees the proxy container's address. Allowing migration through a hostname does not automatically trust that proxy for the administrator claim, and `X-Forwarded-For` is considered only after the direct proxy source is trusted.
