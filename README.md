...
When v0.8.6 configuration needs migration, SeedSync exposes only a read-only migration checkpoint. Direct/package launches bind that checkpoint to `127.0.0.1` unless `--web-bind-host` is explicitly supplied; Docker keeps its explicit `SEEDSYNC_WEB_BIND_HOST` setting. The checkpoint reuses a valid legacy `[Web] port` value from `settings.cfg` (range 1-65535) and otherwise falls back to port 8800; Docker deployments with a custom legacy port must publish that same container port. The retained backup and offline restore path has completed independent end-to-end validation; migration execution and Start remain intentionally disabled until the later product-flow enablement phase.
