...
For legacy deployments that intentionally rely on network-level access control, set the General `disable_browser_auth` setting to `True`, or set Docker's `SEEDSYNC_DISABLE_BROWSER_AUTH=1` (only `0` and `1` are accepted). With this opt-in, anyone who can reach SeedSync can open the ordinary UI and use its non-admin read, write, and stream actions without a browser session or API key; reverse-proxy or network restriction is the operator's responsibility. Admin routes, API-key lifecycle, historical logs, diagnostics, and migration recovery remain protected. Docker applies the environment value on every start, and omitting it or setting `0` restores browser authentication.
