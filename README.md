...
The configuration root mounted at `/config` is a security boundary. New deployments should start the entrypoint as root, set `PUID`/`PGID`, and use a real POSIX directory or named volume that can reach owner-private mode `0700`. That strict path admits only a root-owned root directory or one already owned by the configured `PUID`, rejects initial group/other write access, validates the complete tree without following symlinks, hard-linked regular files, or nested mounts, and limits filesystem types to `ext2`, `ext3`, `ext4`, `xfs`, `btrfs`, `zfs`, `tmpfs`, or `overlay`. Existing Unraid-style containers that set Docker `--user` are handled by a narrower compatibility path: the entrypoint derives that current UID/GID, accepts `fuse.shfs` only when `/config` and every entry already have exactly the same owner, rejects conflicting `PUID`/`PGID`, clamps the root to mode `0700`, and supplies an owner-private NSS identity when that numeric UID is absent from the image so OpenSSH can run normally. This allows an image-only upgrade without a preparatory host `chmod` or `chown`; owner mismatches and unsafe tree entries still fail closed. Other shared, network, and FUSE filesystems remain unsupported. Safe startup assumes one exclusive SeedSync startup with no concurrent same-UID writer or retained write-capable descriptor. Windows shared folders and WSL DrvFS cannot provide this contract; use `compose.windows.yml` or another Docker named volume. Docker administrators and processes that control the Docker daemon remain trusted. This constraint applies only to `/config`, not `/downloads` or `/mounts`.

Migration and the initial browser-claim page are intentionally reachable through any hostname or reverse proxy so a first installation or upgrade needs no network allowlist. The first successful administrator claim closes that temporary window. The first client able to reach and submit the unfinished setup can win that claim, so keep setup on a trusted network and do not publish it to the internet before claiming it. From then on, SeedSync requires a valid API key or an authenticated browser session; browser mutations also retain same-origin checks. Reverse proxies must overwrite client-supplied `X-Forwarded-Host` and `X-Forwarded-Proto` values. HTTPS-proxied browser sessions receive `Secure` cookies; plain-HTTP LAN deployments remain supported.

For legacy deployments that intentionally rely on network-level access control, set the General `disable_browser_auth` setting to `True`, or set Docker's `SEEDSYNC_DISABLE_BROWSER_AUTH=1` (only `0` and `1` are accepted). This is a fully open compatibility mode: anyone who can reach SeedSync can use the ordinary UI and every `/server` route, including admin/API-key lifecycle, historical logs, diagnostics, notifications, admin streams, and migration recovery, without a browser session or API key. Invalid, revoked, or insufficient-scope credentials do not narrow access while this mode is enabled. Network or reverse-proxy isolation is mandatory because users can control the entire application; endpoint input validation, destructive confirmation/attestation, migration eligibility guards, and the existing unsafe-request cross-origin guard still apply (direct clients without browser-origin signals remain supported). Docker applies the environment value on every start, and omitting it or setting `0` restores browser authentication.
