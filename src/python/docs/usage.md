...
### Root-only py-spy support

The runtime image contains the pinned `py-spy` 0.4.2 executable, but the operator wrapper is disabled unless `[General] py_spy_enabled = True` is explicitly configured. The wrapper is root-only (mode 0700) and must be invoked through an explicit `docker exec --user root` action. It accepts only these fixed modes:

* `seedsync-py-spy dump` takes a nonblocking instantaneous thread snapshot, never collects locals, and returns at most 1 MiB of captured dump output after the bounded profiler child exits.
* `seedsync-py-spy record [duration_seconds] [rate_hz]` records a nonblocking speedscope profile, including idle threads, for at most 30 seconds at at most 20 Hz. Both values default to 10 and are validated before the app target is selected. The generated profile is retained only as a root-readable, private file beneath the container's ephemeral `/tmp` storage and is size-limited. The owned profiler child is bounded to the requested duration plus a fixed grace period.

The wrapper resolves exactly one non-root SeedSync process whose script is `/app/python/seedsync.py`; zero or multiple matches fail closed. The configured local filesystem scanner runs inline in that selected Python process, so its local traversal work is in scope for this profile. External processes such as `lftp`, recycled scanner workers, and other subprocess work are not covered; subprocess capture is not implemented. It does not accept arbitrary py-spy options or output paths. Each attempt also leaves root-only, ephemeral `status` and capped `py-spy.stderr` sidecars; failed attempts retain those sidecars but no usable profile, while status reports timeout and stderr truncation. The bounded sidecar may not contain the full profiler diagnostic. Attach failures are reported generically, without forwarding config contents, paths, command lines, or credentials, and the wrapper does not mutate SeedSync state.
