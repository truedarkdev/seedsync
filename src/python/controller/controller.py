...
class Controller:
    ...
    def __transfer_exclude_patterns(
        ...
        _record_fractional_queue_trace(self, file_id, "queue_exclusion_serialization", lambda: {
            ...
            "exact_exclusion_count": len(trusted_patterns),
            "serialization_skipped": serialization_skipped > 0,
    ...
    def __ensure_initial_rescan_deadline(
        ...
        if type(now_monotonic) not in (int, float) or not math.isfinite(float(now_monotonic)):
            return None
        deadline = float(now_monotonic) + self._initial_rescan_timeout_seconds()
    ...
    def __record_incoming_recovery_root_defaults(
        ...
        try:
            ...
            return
        for file_id, sequence, fields in observations:
            try:
                recorder = self.__incoming_recovery_flow_recorder(file_id, sequence, create=False)
