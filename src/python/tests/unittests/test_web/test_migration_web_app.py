...
class TestMigrationWebApp(unittest.TestCase):
    ...
    def test_request_authority_accepts_named_hosts(self) -> None:
        ...
        self.assertEqual(200, self.client.get(
            "/migration", headers={"Host": "seedsync.example.test"},
        ).status_int)
        self.assertEqual(409, self.client.post_json(
    ...
    def test_request_authority_accepts_all_well_formed_hosts(self) -> None:
        for host in (
            "localhost", "127.0.0.1:8800", "192.0.2.20:8800", "[::1]:8800",
            "seedsync.example.test", "seedbox.example:8800",
        ):
            with self.subTest(host=host):
                self.assertEqual(200, self.client.get(
                    "/server/migration/v1/status", headers={"Host": host},
                ).status_int)
        for host in ("127.0.0.1:8800", "192.0.2.20:8800", "seedsync.example.test"):
